...
function LevelRow({ level, index }: { level: Level; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative bg-paper-warm pb-6 pt-8 md:pb-8 md:pt-14"
      >
        ...
        <div className="relative grid items-center gap-8 md:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] md:gap-10 lg:gap-12">
          ...
          <div className={flipped ? "md:order-1" : ""}>
            <header className="mb-4 md:mb-5">
              ...
              <h4 className="mt-2 text-card font-serif font-normal text-ink">
                {level.label}
              </h4>
              <p className="mt-1.5 body-quiet">{level.subhead}</p>
            </header>
            ...
            <ul className="mt-3 space-y-2 md:mt-4">
              {level.bullets.map((bullet) => (
                <li
                  key={bullet}
                  className="flex gap-2.5 text-[14.5px] leading-snug text-ink-soft md:text-[15px]"
                >
                  <span
                    aria-hidden
...
function LevelImage({
  ...
  return (
    <div
      className="relative aspect-[4/5] w-full max-w-[240px] overflow-hidden rounded-2xl bg-ink/5 md:max-w-[280px] lg:max-w-[320px]"
      style={{
        opacity: active ? 1 : 0,
