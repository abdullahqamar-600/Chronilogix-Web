...
export function BrokersTrust() {
  const { ref, inView } = useReveal<HTMLDivElement>();

  return (
    <section
      aria-labelledby="brokers-trust-label"
    ...
    >
      ...
      >
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:items-center lg:gap-16">
          ...
          <div
            className="reveal-row rounded-[24px] border border-ink/10 bg-white p-8 md:p-10 [transition-delay:520ms]"
          >
            <p className="eyebrow-muted">Compliance posture</p>
            <div className="mt-5">
              <ComplianceBadges />
            </div>
            <p className="mt-6 max-w-[42ch] body-quiet">
              The certifications your CFO and legal counsel will ask about
              before the pilot &mdash; ready in one link, not scattered
              across seven vendor pages.
            </p>
          </div>
        </div>
      </div>
