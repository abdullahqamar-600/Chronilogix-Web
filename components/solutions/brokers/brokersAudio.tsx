...
const AUDIO_SRC = "/audio/chronilogix-broker-track.mp3";
const TRACK_TITLE = "The broker brief";
...
export function BrokersAudioProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AudioCtx>(
    ...
    [isPlaying, currentTime, duration, activated, toggle, activateAndPlay, dismiss, seekTo],
  );
...
export function BrokersStickyAudio() {
  const { activated, isPlaying, currentTime, duration, toggle, dismiss, seekTo } =
    useBrokersAudio();
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-auto flex w-full max-w-[760px] flex-col overflow-hidden rounded-2xl border border-ink/8 bg-white/95 shadow-[0_2px_6px_rgba(15,20,25,0.06),0_24px_60px_-24px_rgba(15,20,25,0.28)] backdrop-blur-md">
        {isPlaying ? (
          <TranscriptPanel currentTime={currentTime} onSeek={seekTo} />
        ) : null}
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 md:gap-4 md:px-4 md:py-3">
          ...
          <button
            ...
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-ink-muted transition-colors duration-200 ease-out hover:bg-ink/5 hover:text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent/50"
          >
